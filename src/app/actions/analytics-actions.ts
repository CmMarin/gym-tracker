...
function resolveMuscleGroups(exerciseName?: string | null, category?: string | null, customTargetMuscles?: string[] | null): string[] {
  ...
  if (name.includes("squat") || (name.includes("press") && name.includes("leg"))) return ["Legs", "Glutes"];
  if (name.includes("deadlift") || name.includes("row") || name.includes("pull up") || name.includes("pulldown") || name.includes("pull-up")) return ["Back"];
  if (name.includes("curl") && !name.includes("leg")) return ["Biceps"];
  if (name.includes("extension") || name.includes("tricep") || name.includes("pushdown") || name.includes("skullcrusher")) return ["Triceps"];
  if (name.includes("crunch") || name.includes("plank") || name.includes("sit up")) return ["Core"];
  if (name.includes("lateral") || name.includes("raise") || name.includes("shoulder") || name.includes("military") || name.includes("overhead")) return ["Shoulders"];
  
  // Broader chest checking (after we've ruled out overhead/shoulder/leg presses)
  if (name.includes("chest") || name.includes("bench") || name.includes("fly") || name.includes("push up") || name.includes("pec") || name.includes("press")) return ["Chest"];
...
export async function getMuscleFatigue() {
  ...
  recentSets.forEach(set => {
    ...
    const daysAgo = (new Date().getTime() - new Date(set.createdAt).getTime()) / (1000 * 3600 * 24);
