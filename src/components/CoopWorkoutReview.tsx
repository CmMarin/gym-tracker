...
export default function CoopWorkoutReview({
  ...
  if (!sessionData)
    return (
      <div className="text-[var(--color-white)] animate-pulse font-bold mt-10">
        Loading review...
      </div>
    );
  ...
  // Calculate stats
  // Highest Volume
  const topVolumeMember = [...sessionData.members].sort(
    (a, b) => b.volume - a.volume,
  )[0];
  // Most PRs
  const mostPrsMember = [...sessionData.members].sort(
    (a, b) => b.prsCount - a.prsCount,
  )[0];
  // Most Penalties
  const mostPenaltiesMember = [...sessionData.members].sort(
    (a, b) => b.penalties - a.penalties,
  )[0];
  ...
  return (
    ...
    >
      <PartyPopper
        size={80}
        className="mx-auto mb-4 text-yellow-300 drop-shadow-lg"
      />
      <h2 className="text-4xl font-black mb-2">Team Review</h2>

      {!allFinished ? (
        <p className="text-indigo-200 font-bold mb-6 animate-pulse">
          Waiting for teammates to finish...
        </p>
      ) : (
        <p className="text-green-300 font-black mb-6 uppercase tracking-wider text-sm">
          Everyone is done! ??
        </p>
      )}
        ...
        {topVolumeMember && topVolumeMember.volume > 0 && (
          <div className="bg-indigo-700/50 rounded-2xl p-4 flex items-center justify-between border-2 border-indigo-500">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-indigo-200 uppercase font-black tracking-wider">
                  Heavy Lifter
                </p>
                <p className="font-bold text-lg leading-tight">
                  {topVolumeMember.user.username}
                </p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-black">
                {topVolumeMember.volume.toLocaleString()}{" "}
                <span className="text-sm font-bold text-indigo-300">kg</span>
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-indigo-700/50 rounded-2xl p-4 flex items-center justify-between border-2 border-indigo-500">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-lg leading-tight">
                  {mostPrsMember.user.username}
                </p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-black">
                {mostPrsMember.prsCount}{" "}
                <span className="text-sm font-bold text-indigo-300">PRs</span>
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-indigo-700/50 rounded-2xl p-4 flex items-center justify-between border-2 border-indigo-500">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-lg leading-tight">
                  {mostPenaltiesMember.user.username}
                </p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-black text-red-300">
                {mostPenaltiesMember.penalties}{" "}
                <span className="text-sm font-bold text-red-400">Slips</span>
              </p>
            </div>
          </div>
        )}
        ...
        <div className="relative z-10">
          ...
          <div className="flex justify-between items-center mb-2">
            <span className="text-3xl font-black">
              {sessionData.totalXp}{" "}
              <span className="text-lg text-indigo-300">PT</span>
            </span>
          ...
          <div className="w-full h-3 bg-indigo-900 rounded-full overflow-hidden">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{
                width: `${Math.min(100, (sessionData.totalXp / sessionData.goalXp) * 100)}%`,
              }}
            />
          </div>
          {sessionData.totalXp >= sessionData.goalXp && (
            <p className="mt-3 text-sm font-black text-green-300 text-center uppercase tracking-widest">
              +200 XP BONUS UNLOCKED!
            </p>
          )}
        </div>
      </div>
