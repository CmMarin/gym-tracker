...
export default function CoopWorkoutReview({ sessionId }: { sessionId: string }) {
  ...
  return (
    <motion.div
      ...
      className="bg-blue-600 p-8 rounded-[2rem] shadow-2xl w-full max-w-md text-center text-white border-b-8 border-blue-800 mt-10"
    >
      ...
      {!allFinished ? (
        <p className="text-blue-200 font-bold mb-6 animate-pulse">Waiting for teammates to finish...</p>
      ) : (
      ...
      <div className="space-y-3 text-left">
        {topVolumeMember && topVolumeMember.volume > 0 && (
          <div className="bg-blue-700/50 rounded-2xl p-4 flex items-center justify-between border-2 border-blue-500">
            <div className="flex items-center gap-3">
              <Dumbbell className="text-orange-400" size={28} />
              <div>
                <p className="text-[10px] text-blue-200 uppercase font-black tracking-wider">Heavy Lifter</p>
                <p className="font-bold text-lg leading-tight">{topVolumeMember.user.username}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-black">{topVolumeMember.volume.toLocaleString()} <span className="text-sm font-bold text-blue-300">kg</span></p>
            </div>
          </div>
        )}
        ...
        {mostPrsMember && mostPrsMember.prsCount > 0 && (
          <div className="bg-blue-700/50 rounded-2xl p-4 flex items-center justify-between border-2 border-blue-500">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-blue-200 uppercase font-black tracking-wider">PR Machine</p>
                <p className="font-bold text-lg leading-tight">{mostPrsMember.user.username}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-black">{mostPrsMember.prsCount} <span className="text-sm font-bold text-blue-300">PRs</span></p>
            </div>
          </div>
        )}
        ...
        {mostPenaltiesMember && mostPenaltiesMember.penalties > 0 && (
          <div className="bg-blue-700/50 rounded-2xl p-4 flex items-center justify-between border-2 border-blue-500">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-blue-200 uppercase font-black tracking-wider">Most Slack</p>
      ...
      <div className="mt-6 bg-blue-800 rounded-2xl p-4 mb-6 relative overflow-hidden">
        <div className="relative z-10">
          <p className="text-sm font-bold text-blue-200 mb-1">Team Goal Progress</p>
          <div className="flex justify-between items-center mb-2">
            <span className="text-3xl font-black">{sessionData.totalXp} <span className="text-lg text-blue-300">PT</span></span>
            <span className="text-lg font-bold text-blue-300">/ {sessionData.goalXp}</span>
          </div>
      ...
      <button
        ...
        className="bg-white text-blue-600 font-black text-xl w-full py-4 rounded-2xl shadow-[0_6px_0_0_#bfdbfe] active:translate-y-[6px] active:shadow-none transition-all"
      >
        BACK TO DASHBOARD
      </button>
    </motion.div>
