    ...
    <div className="w-full max-w-md bg-[var(--color-white)] border-2 border-indigo-100 rounded-3xl p-5 mb-6 shadow-[0_4px_0_0_var(--color-theme-shadow)]">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-bold text-slate-800 flex items-center gap-2">
          <Users size={20} className="text-indigo-500" /> Co-Op:{" "}
          {sessionData.name}
        </h3>
      ...
      <div className="space-y-3 mb-4">
        {sessionData.members.map((m: any) => (
          <div
            key={m.id}
            className="flex items-center justify-between text-sm bg-gray-50 p-2 rounded-xl"
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="font-bold text-slate-700 block">
                  {m.user.username}{" "}
                  <span className="text-indigo-500">+{m.xpContributed} XP</span>
                </span>
                <span className="text-xs text-slate-400 font-medium">
                  {m.status === "RESTING"
                    ? "Resting..."
                    : String(m.currentExercise || "Starting out").substring(
                        0,
                        15,
                      )}
                </span>
